...
IMG_DIR = os.path.join(WS_DIR, 'data', 'images')

CALIBRATION_FILES = {
    'right': os.path.join(DATA_DIR, 'calibration_data_right.json'),
    'left': os.path.join(DATA_DIR, 'calibration_data_left.json'),
}
...
CAMERA_CONFIG = {
    'right': {
        ...
        'output': os.path.join(IMG_DIR, 'detection_result.png'),
    },
    'left': {
        'camera_selection': 1,
        'image_topic': '/zedxmini1/zed_node/left/image_rect_color',
        'label': 'Left Camera (zedxmini1)',
        'output': os.path.join(IMG_DIR, 'detection_result_left.png'),
    },
}
...
class DetectionVisualizer(Node):
    ...
    def _save_combined(self, multi=False):
        ...
        if left_img is not None and right_img is not None:
            ...
            combined_path = os.path.join(IMG_DIR, f'detection_result_both{suffix}.png')
