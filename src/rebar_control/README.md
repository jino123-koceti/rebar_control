...
### 연속 제어 (아날로그 조이스틱) - 실제 리모콘 조작 기준
- **AN3 (조이스틱 3)**: 하부체 전후진 (0x141, 0x142) - 코드상 angular 변수 (모터 180도 장착)
- **AN4 (조이스틱 4)**: 하부체 좌우회전 (0x141, 0x142) - 코드상 linear 변수 (모터 180도 장착)
...
- **중립**: 대기
- **S20 (하단)**: Automatic Control 모드 (상위제어 연동 + 호밍)
...
### Yaw 회전 / 스테이지 호밍 (3단 토글 스위치 - 엣지 트리거)
- **S23 ON→OFF** (S19 Remote 모드): 공구 Yaw +30도 회전 (0x147)
- **S23 ON→OFF** (S20 Auto 모드): **스테이지 XYZ 호밍**
  - 0x144 (X축): +방향→IN03 감지 시 정지
  - 0x145 (Y축): -방향→IN01 감지 시 정지
  - 0x147 (Yaw): +방향→IN04 감지 시 정지
  - 3축 모두 홈 도달 시 엔코더=0° 리셋
